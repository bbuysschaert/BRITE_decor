# -*- coding: utf-8 -*-
"""
Routines to detrend for the position - flux instrumental effects
...
from BRITE.plotting.PLOTdetrendPositionFlux import PLOTdetrendPOSITIONfluxFULL, PLOTdetrendPOSITIONfluxDIAGinformCRIT
#===============================================================================#
...
# 				Code
#===============================================================================
def detrendPOSITIONflux(time, position, flux, **kwargs):
    """
    Routine to perform the detrending of the instrumental effect seen between the CCD position (one axis) and BRITE flux.
    
    We call the general routine BRITE.detrending.detrendPARAMflux and use position as the param. Next, we update the kwarg dictionary, so appropriate settings are achieved, even when the user did not specify anything for the kwargs.
    ...
    @kwargs: SPLINEknotpointsSPACING: set of spacing for the different knotpoints - Default is np.array([0.5, 1.0, 2.0]) [pixel] WARNING in case you detrend for the PSF changes, keep your knotpoint spacing larger than what you used there...
    ...
    SPLINEknotpointsSPACING = kwargs.get('SPLINEknotpointsSPACING', np.array([0.5, 1.0, 2.0])) #[pixel]
    ...
    # Doing the fitting
    matrixAIC, matrixBIC, likelihoodMATRIX, matrixTCK = detrendPARAMflux(position, flux, **kwargs)
	...
	PLOTdetrendPOSITIONfluxDIAGinformCRIT(flux, position, TCKaic, TCKbic)
	pl.show()
    ...
    if show_ME or show_FITS:
      PLOTdetrendPOSITIONfluxFULL(time, flux, position, TCKoptimal)
      pl.show()
