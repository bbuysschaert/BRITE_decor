# -*- coding: utf-8 -*-
"""
Routines to detrend for the satellite orbital phase - flux instrumental effects
...
from BRITE.fitting.splinefit import splineFIT, splineGOODNESSofFITandINFORMATIONCRITERION, reconvertTCKfromSTRING
from BRITE.detrending.detrendPARAMFlux import detrendPARAMflux
from BRITE.plotting.PLOTdetrendOrbitFlux import PLOTdetrendORBITfluxFULL, PLOTdetrendORBITfluxDIAGinformCRIT
#===============================================================================#
...
# 				Code
#===============================================================================
def detrendORBITflux(time, orbitalPHASE, flux, **kwargs):
    """
    Routine to perform the detrending of the instrumental effect seen between the satellite's orbital phase and BRITE flux.
    
    We call the general routine BRITE.detrending.detrendPARAMflux and use orbitalPHASE as the param. Next, we update the kwarg dictionary, so appropriate settings are achieved, even when the user did not specify anything for the kwargs.
    
    NOTE the fit should happen periodic, in case you reach both a phase = 0 and a phase = .99
    ...
    @kwargs: SPLINEknotpointsSPACING: set of spacing for the different knotpoints - Default is np.array([.05, .1, .2, .25, 1./3.]) []
    @kwargs: SPLINEphaseSHIFT: value to consider for the phase shift for the same sets of knotpoints - Default is 0.005 []
    ...
    SPLINEknotpointsSPACING = kwargs.get('SPLINEknotpointsSPACING', np.array([.05, .1, .2, .25, 1./3.])) #[]
    SPLINEphaseSHIFT = kwargs.get('SPLINEphaseSHIFT', 0.005) #[]
    ...
    # Check if you cross over orbitalPHASE = 1. If so, you have to perform a periodic fit
    lowerPHASE, upperPHASE = np.around(np.nanmin(orbitalPHASE),decimals=2), np.around(np.nanmax(orbitalPHASE),decimals=2)
    if lowerPHASE <= .02 and upperPHASE >= .98:
      kwargs['SPLINEperiodic'] = True 
      print bcolors.OKBLUE + '\t\detrendORBITflux: Performing periodic fit...' + bcolors.ENDC
    
    # Doing the fitting
    matrixAIC, matrixBIC, likelihoodMATRIX, matrixTCK = detrendPARAMflux(orbitalPHASE, flux, **kwargs)
	...
	PLOTdetrendORBITfluxDIAGinformCRIT(flux, orbitalPHASE, TCKaic, TCKbic)
	pl.show()
    ...
    if show_ME or show_FITS:
      PLOTdetrendORBITfluxFULL(time, flux, orbitalPHASE, TCKoptimal)
      pl.show()
